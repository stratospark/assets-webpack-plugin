  ...
  return function writeOutput (newAssets, next) {
    ...
    mkdirp(options.path, function (err) {
      ...
      fs.readFile(outputPath, 'utf8', function (err, data) {
        ...
        // TODO: Pass in template file name as parameter and interpolate manifest into it
        output = "var manifest = " + output + ";\n\n";
        output = output + "var s1=document.createElement("script");s1.src=manifest.entries.vendor.js,document.getElementsByTagName("body")[0].appendChild(s1);var s2=document.createElement("script");s2.src=manifest.entries.main.js,document.getElementsByTagName("body")[0].appendChild(s1);var fc_css=document.createElement("link");fc_css.setAttribute("rel","stylesheet"),fc_css.setAttribute("media","screen"),fc_css.setAttribute("href",manifest.entries.main.css),document.getElementsByTagName("body")[0].appendChild(fc_css);"
